...
import { updateMemberScopeSchema } from "@/contexts/workforce/interfaces/rest/schemas/workforce-member.schemas";
...
export async function PUT(request: Request, { params }: { params: Promise<{ memberId: string }> }) {
  ...
  const parsed = updateMemberScopeSchema.safeParse(await request.json().catch(() => undefined));
  if (!parsed.success) {
    return NextResponse.json({ message: parsed.error.issues[0]?.message ?? "Invalid request" }, { status: 400 });
  }

  try {
    await new WorkforceApiGateway().updateMemberScope(organizationId!, memberId, parsed.data.establishmentIds);
