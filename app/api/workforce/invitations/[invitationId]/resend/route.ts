...
export async function POST(request: Request, { params }: { params: Promise<{ invitationId: string }> }) {
  ...
  if (!z.string().uuid().safeParse(organizationId).success || !z.string().uuid().safeParse(invitationId).success) {
    return NextResponse.json({ message: "A valid organization and invitation are required" }, { status: 400 });
  }

  try {
    const invitation = await new WorkforceApiGateway().resendInvitation(organizationId!, invitationId);
